...
TSharedRef<SDockTab> FAudiokineticToolsModule::CreateWwisePickerGroupTab(const FSpawnTabArgs& SpawnTabArgs)
{
...
void FAudiokineticToolsModule::VerifyGeneratedSoundBanksPath(UAkSettings* AkSettings, UAkSettingsPerUser* AkSettingsPerUser)
{
	if (!AkSettings->GeneratedSoundBanksPathExists())
	{
		...
		{
			UE_LOG(LogAudiokineticTools, Log, TEXT("GeneratedSoundBanks folder not found. The Wwise picker will not be usable."));
		}
	}
	else
	{
