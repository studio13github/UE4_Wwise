	...
	const int32 TransitionDuration, const EAkCurveInterpolation FadeCurve)
{
	...
	{
		UE_LOG(LogAkAudio, Warning, TEXT("Failed to execute an action on AkAudioEvent '%s' without an Audio Device."), *GetName());
	...
	FWaitEndOfEventAction* LatentAction, const EAkAudioContext AudioContext)
{
	...
	if (UNLIKELY(!AudioDevice))
	{
		UE_LOG(LogAkAudio, Warning, TEXT("Failed to post AkAudioEvent '%s' at a location without an Audio Device."), *GetName());
		return AK_INVALID_PLAYING_ID;
	}
	...
	{
		UE_LOG(LogAkAudio, Log, TEXT("No Spatial Audio Room while PostOnLocation AkAudioEvent '%s' (ObjId: %" PRIu64 ")"), *GetName(), ObjectID);
	}
