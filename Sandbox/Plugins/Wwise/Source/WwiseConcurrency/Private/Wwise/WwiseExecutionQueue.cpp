...
void FWwiseExecutionQueue::StartWorkerIfNeeded()
{
	if (TrySetStoppedWorkerToRunning())
	{
		...
		{
			UE_CLOG(DEBUG_WWISEEXECUTIONQUEUE, LogWwiseConcurrency, VeryVerbose, TEXT("FWwiseExecutionQueue::StartWorkerIfNeeded (%p): Concurrency is not available. Starting new worker thread on Task Graph"), this);
...
bool FWwiseExecutionQueue::StopWorkerIfDone()
{
	...
	else if (LIKELY(TrySetClosingWorkerToClosed()))
	{
		...
		if (bDeleteOnceClosedCopy)		// We use a copy since the deletion might've already occurred
		{
			FFunctionGraphTask::CreateAndDispatchWhenReady([this]
			{
				UE_LOG(LogWwiseConcurrency, VeryVerbose, TEXT("FWwiseExecutionQueue::StopWorkerIfDone (%p): Auto deleting on Game Thread"), this);
				delete this;
