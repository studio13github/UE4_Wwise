...
public class WwiseSoundEngineVersion
{
	...
	{
		return string.Join("_", ClassName.Split('_').Skip(1).ToArray());
	}

	public static string[] GetVersionDefinesFromClassName(string ClassName)
	{
		var VersionNumber = GetVersionFromClassName(ClassName);
		return new[]
		{
			string.Format("AK_WWISE_SOUNDENGINE_VERSION=\"{0}\"", VersionNumber.Replace('_', '.')),
			string.Format("AK_WWISE_SOUNDENGINE_{0}", VersionNumber)
		};
	}
